...
class Assignment2Activity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        view.bnvListGrid.setOnItemSelectedListener {
            when (it.itemId) {
                R.id.item_list -> Fragmentmodify(FragmentList())
                R.id.item_grid -> Fragmentmodify(FragmentGrid())
                else -> false
            }
        }
    }

    private fun Fragmentmodify(fragment: Fragment): Boolean {
        supportFragmentManager
            .beginTransaction()
            .replace(R.id.fcLayoutGrid,fragment)
